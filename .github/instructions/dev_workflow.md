...
-   Configure rule sets during initialization with `--rules` flag (e.g., `task-master init --rules cursor`) or manage them later with `task-master rules add/remove` commands  
...
- **Available Profiles**: Cursor (cursor)
- **During Initialization**: Use `task-master init --rules cursor` to specify rule set to include
